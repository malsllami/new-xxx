...
// تسجيل بصمة المدير (بعد دخوله بالرمز)
async function registerAdminBiometric(adminCode) {
  if (!isWebAuthnSupported()) {
    return { success: false, message: 'هذا المتصفح/الجهاز لا يدعم البصمة الحيوية' };
  }

  const challengeRes = await callApi('webauthnAdminRegisterChallenge', { adminCode });
  if (!challengeRes.success) return challengeRes;
  ...
  try {
    ...
    return await callApi('webauthnAdminRegisterVerify', {
      adminCode,
      credentialId:      bufferToBase64Url(credential.rawId),
      attestationObject: bufferToBase64Url(credential.response.attestationObject),
      clientDataJSON:    bufferToBase64Url(credential.response.clientDataJSON),
...
// تسجيل دخول المدير بالبصمة (بدون رمز)
async function loginAdminWithBiometric() {
  if (!isWebAuthnSupported()) {
    return { success: false, message: 'هذا المتصفح/الجهاز لا يدعم البصمة الحيوية' };
  }

  const challengeRes = await callApi('webauthnAdminLoginChallenge', {});
  if (!challengeRes.success) return challengeRes;
  ...
  try {
    ...
    return await callApi('webauthnAdminLoginVerify', {
      credentialId:      bufferToBase64Url(credential.rawId),
      authenticatorData: bufferToBase64Url(credential.response.authenticatorData),
      signature:         bufferToBase64Url(credential.response.signature),
      clientDataJSON:    bufferToBase64Url(credential.response.clientDataJSON)
